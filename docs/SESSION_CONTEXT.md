...
> Updated at 14 June 2026 — Site live on hrishi.org.in — Vercel Blob image proxy — Adapter switch to adapter-auto
...
> **Note:** PII images (monogram.webp, monogram.png, og-image.svg) removed from repo. Now served via `/api/images/` proxy from private Vercel Blob store.
```
...
- `PUBLIC_IMAGE_MONOGRAM` — `/api/images/monogram.webp` (served via blob proxy)
- `PUBLIC_IMAGE_MONOGRAM_FALLBACK` — `/api/images/monogram.png`
- `PUBLIC_IMAGE_OG` — `/api/images/og-image.svg`
